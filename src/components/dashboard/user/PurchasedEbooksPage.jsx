      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {filteredEbooks.map((book) => (
            <Card
              key={book._id}
              className="group bg-white border border-slate-100 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-300 flex flex-col h-full rounded-2xl overflow-hidden"
            >
              {/* Cover Image Wrapper inside Card.Content */}
              <Card.Content className="p-0 relative aspect-[3/4] w-full bg-slate-100 overflow-hidden">
                {book.coverImage ? (
                  <img
                    src={book.coverImage}
                  ...
                  // Fallback fallback decorative background if cover image missing
                  <div className="w-full h-full bg-gradient-to-br from-indigo-50 to-slate-100 flex flex-col items-center justify-center p-4 text-center">
                    <BookOpen className="w-10 h-10 text-indigo-300 mb-2 animate-pulse" />
              ...
              <Card.Header className="p-4 flex-1 flex flex-col items-start gap-1">
                <Card.Title className="text-base font-bold text-slate-800 line-clamp-1 group-hover:text-indigo-600 transition-colors">
                  {book.ebookTitle}
                </Card.Title>
              ...
              {/* Action Button Link */}
              <Card.Footer className="p-4 pt-0 border-t border-slate-50 mt-auto">
                <Link
                  href={`/ebooks/${book._id}`}
                  className="w-full inline-flex items-center justify-center gap-1.5 py-2 px-3 text-xs font-semibold text-indigo-600 bg-indigo-50/50 hover:bg-indigo-50 rounded-xl transition-all group/btn"
                >
                  View Details
